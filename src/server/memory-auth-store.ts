...
export class MemoryAuthStore implements AuthStore {
  ...
  async findMembership(userId: string, organizationId?: string): Promise<Membership | null> {
    return this.memberships.find((item) => item.userId === userId && (!organizationId || item.organizationId === organizationId)) ?? null;
  }
  async createSession(session: StoredSession): Promise<void> { this.sessions.set(session.id, session); }
  async findSession(id: string): Promise<StoredSession | null> { return this.sessions.get(id) ?? null; }
  async revokeSession(id: string, revokedAt: Date): Promise<void> {
    const session = this.sessions.get(id);
    if (session) this.sessions.set(id, { ...session, revokedAt });
  }
  async listMemberships(userId: string): Promise<Membership[]> { return this.memberships.filter((item) => item.userId === userId); }
  async removeMembership(userId: string, organizationId: string): Promise<void> {
    const index = this.memberships.findIndex((item) => item.userId === userId && item.organizationId === organizationId);
