...
export class AuthService {
  ...
  async removeMember(actor: AuthenticatedUser, userId: string): Promise<void> {
    ...
    if (membership.role === 'owner') {
      const owners = (await this.store.listMembershipsForOrganization(actor.organizationId)).filter(
        (candidate) => candidate.role === 'owner',
      );
      if (owners.length <= 1)
        throw new AuthorizationError('An organization must retain at least one owner.');
    }
