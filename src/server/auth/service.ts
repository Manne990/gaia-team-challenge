...
export class AuthService {
  ...
  async signIn(email: string, password: string, organizationId?: string): Promise<{ token: string; identity: SessionIdentity }> {
    ...
    const user = this.db.prepare(`
      SELECT id, email, password_hash AS passwordHash, display_name AS displayName, disabled_at AS disabledAt
      FROM users WHERE email = ? COLLATE NOCASE
    `).get(normalizedEmail) as { id: string; email: string; passwordHash: string; displayName: string; disabledAt: string | null } | undefined;

    let passwordValid = false;
    try {
      passwordValid = await argonVerify(user?.passwordHash ?? DUMMY_HASH, password);
    } catch {
      passwordValid = false;
    }
    if (!user || !passwordValid || user.disabledAt) throw new AuthenticationError(GENERIC_SIGN_IN_ERROR);
    ...
    `).get(user.id, organizationId ?? null, organizationId ?? null) as { organizationId: string; role: Role } | undefined;
    ...
    this.db.prepare(`
      INSERT INTO sessions (id_hash, user_id, organization_id, created_at, expires_at)
      VALUES (?, ?, ?, ?, ?)
    `).run(digest(token), user.id, membership.organizationId, iso(createdAt), iso(expiresAt));

    return {
      token,
      identity: {
        sessionHash: digest(token), userId: user.id, organizationId: membership.organizationId,
  ...
  authenticate(token: string | undefined): SessionIdentity {
    ...
    const row = this.db.prepare(`
      SELECT s.id_hash AS sessionHash, s.user_id AS userId, s.organization_id AS organizationId,
             s.expires_at AS expiresAt, m.role, u.email, u.display_name AS displayName
      FROM sessions s
  ...
  logout(token: string | undefined): void {
    ...
    this.db.prepare("UPDATE sessions SET revoked_at = COALESCE(revoked_at, ?) WHERE id_hash = ?")
  ...
  addMembership(actor: SessionIdentity, input: { userId: string; role: Role }): void {
    this.requireRole(actor, "owner");
    this.db.prepare(`INSERT INTO memberships (organization_id, user_id, role, created_at) VALUES (?, ?, ?, ?)`)
      .run(actor.organizationId, input.userId, input.role, iso(this.now()));
  }
  ...
  updateMembership(actor: SessionIdentity, userId: string, role: Role): void {
    ...
    this.db.transaction(() => {
      ...
      this.db.prepare("UPDATE memberships SET role = ? WHERE organization_id = ? AND user_id = ?")
        .run(role, actor.organizationId, userId);
