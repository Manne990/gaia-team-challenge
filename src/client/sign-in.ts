...
export function mountSignInForm(root: HTMLElement, onAuthenticated: () => void): void {
  ...
  form.addEventListener("submit", async (event) => {
    ...
    try {
      const response = await fetch("/api/auth/sign-in", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(Object.fromEntries(new FormData(form))) });
      const data = await response.json() as { error?: string };
      if (!response.ok) throw new Error(data.error ?? "We could not sign you in. Please try again.");
      onAuthenticated();
    } catch (error) {
      feedback.textContent = error instanceof Error ? error.message : "We could not sign you in. Please try again.";
    } finally {
