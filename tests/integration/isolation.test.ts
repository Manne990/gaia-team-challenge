...
import { createTemporaryDatabase, expectRejectedWithoutForeignMutation, reserveUniquePort } from "../support/isolation.js";

describe("isolated test resources", () => {
  it("allocates independent ports and removes temporary databases idempotently", async () => {
    const [firstPort, secondPort] = await Promise.all([reserveUniquePort(), reserveUniquePort()]);
