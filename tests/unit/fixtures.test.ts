...
describe("deterministic CRM fixture", () => {
  it("covers isolation, permissions, duplicate labels, history, stages, due states, and pagination", () => {
    ...
    expect(new Set(crmFixture.users.map(({ role }) => role))).toEqual(
      new Set(["owner", "member", "viewer"]),
    );
    expect(
      crmFixture.companies.filter(({ name }) => name === "Acme Group"),
    ).toHaveLength(3);
    expect(
      crmFixture.activities.some(({ occurredAt }) => occurredAt < "2026-01-01"),
    ).toBe(true);
    expect(crmFixture.pipelineStages).toHaveLength(3);
    expect(crmFixture.tasks.map(({ id }) => id)).toEqual([
      "task_overdue",
      "task_upcoming",
      "task_completed",
    ]);
    expect(
      crmFixture.companies.filter(
        ({ organizationId }) => organizationId === "org_northstar_demo",
      ).length,
    ).toBeGreaterThan(25);
  });
