...
async function fixture() {
  ...
  store.users.set('user-2', {
    ...
    passwordHash: await hashPassword('ViewerPass!2026'),
  });
  ...
  return {
    store,
    auth: new AuthService(store, () => now),
    setNow: (value: Date) => {
      now = value;
    },
  };
...
test('sign-in uses a generic failure for unknown account and wrong password', async () => {
  ...
  await assert.rejects(() => auth.signIn('missing@example.test', 'anything'), {
    message: 'Invalid email or password.',
  });
  await assert.rejects(() => auth.signIn('owner@northstar.test', 'incorrect'), {
    message: 'Invalid email or password.',
  });
});
...
test('only owners can administer membership and the last owner remains', async () => {
  ...
  const owner = await auth.authenticate(
    (await auth.signIn('owner@northstar.test', 'OwnerPass!2026')).token,
  );
  await assert.rejects(() => auth.removeMember(owner!, 'user-1'), {
    message: 'An organization must retain at least one owner.',
  });
  await assert.rejects(
    () => auth.removeMember({ ...owner!, role: 'member' }, 'user-2'),
    AuthorizationError,
  );
...
test('HTTP routes issue an HttpOnly session cookie and clear it on logout', async () => {
  ...
  const signIn = await handleAuthRequest(
    new Request('http://crm.test/api/auth/sign-in', {
      method: 'POST',
