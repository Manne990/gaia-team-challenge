...
test('owner can sign in, reach CRM records, and use the primary workflow accessibly', async ({
  page,
}) => {
...
test('a signed-in user cannot discover or mutate a foreign company', async ({ page }) => {
  ...
  try {
    ...
    const mutation = await page.request.put(`${server.url}/api/companies/cmp_outside_acme`, {
      data: { name: 'Mutated' },
    });
