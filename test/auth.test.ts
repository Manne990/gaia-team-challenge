...
describe("authentication", () => {
  it("accepts deterministic seeded scrypt credentials during the Argon2 transition", async () => {
    ...
    await assert.doesNotReject(() =>
      auth.signIn("owner@northstar.test", "OwnerPass!2026"),
    );
  });
  ...
  it("creates an organization-bound session and stores only its digest", async () => {
    ...
    const persisted = db
      .prepare("SELECT token_hash AS hash FROM sessions")
      .get() as { hash: string };
    assert.notEqual(persisted.hash, result.token);
    assert.deepEqual(auth.authenticate(result.token).role, "owner");
    assert(
      !readFileSync(join(directory, "test.sqlite")).includes(
        Buffer.from(result.token),
      ),
    );
  });
...
describe("authorization and tenant isolation", () => {
  it("enforces the owner, member, and viewer capability ordering", () => {
    ...
    assert.doesNotThrow(() =>
      auth.requireRole({ ...owner, role: "member" }, "member"),
    );
    assert.throws(
      () => auth.requireRole({ ...owner, role: "viewer" }, "member"),
      AuthorizationError,
    );
    assert.throws(
      () => auth.requireRole({ ...owner, role: "member" }, "owner"),
      AuthorizationError,
    );
  });
  ...
  it("does not change foreign persisted state when an identifier is guessed", () => {
    const before = db
      .prepare("SELECT * FROM memberships WHERE organization_id = 'org-b'")
      .all();
    assert.throws(
      () => auth.removeMembership(owner, "user-outside"),
      AuthorizationError,
    );
    const after = db
      .prepare("SELECT * FROM memberships WHERE organization_id = 'org-b'")
  ...
  it("restricts membership administration to owners", () => {
    assert.throws(
      () =>
        auth.addMembership(
          { ...owner, role: "member" },
          { userId: "user-outside", role: "viewer" },
        ),
      AuthorizationError,
    );
    assert.throws(
  ...
  it("lists only the authenticated owner's organization members", () => {
    ...
    assert(
      !members.some((member) => member.email === "other-owner@outside.test"),
    );
  });
  ...
  it("prevents removal or demotion of the last owner transactionally", () => {
    ...
    assert.throws(
      () => auth.updateMembership(owner, "user-owner", "member"),
      MembershipConflictError,
    );
    assert.equal(
      (
        db
          .prepare("SELECT role FROM memberships WHERE user_id = 'user-owner'")
          .get() as { role: string }
      ).role,
      "owner",
    );
  ...
  it("allows owner transfer and revokes sessions when access is reduced", async () => {
    ...
    assert.equal(
      typeof (
        db
          .prepare(
            "SELECT removed_at AS removedAt FROM memberships WHERE user_id = 'user-owner'",
          )
          .get() as { removedAt: string }
      ).removedAt,
      "string",
    );
    await assert.rejects(
    ...
    const memberSession = await auth.signIn(
      "viewer@northstar.test",
      "OwnerPass!2026",
    );
    auth.updateMembership(newOwner, "user-viewer", "viewer");
    assert.throws(
      () => auth.authenticate(memberSession.token),
      AuthenticationError,
    );
    const actions = db
    ...
    assert(
      actions.some(({ action }) => action === "membership.sessions_revoked"),
    );
  });
});
...
describe("authentication HTTP boundary", () => {
  it("sets an HTTP-only cookie and revokes it through logout", async () => {
    ...
    try {
      ...
      const revoked = await fetch(`${base}/api/auth/session`, {
        headers: { cookie: cookie!.split(";")[0]! },
      });
  ...
  it("rejects cross-origin state changes before credentials are processed", async () => {
    ...
    try {
      ...
      const response = await fetch(
        `http://127.0.0.1:${address.port}/api/auth/sign-in`,
        {
          method: "POST",
          headers: {
