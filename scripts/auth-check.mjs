...
import { createAuthService, AuthError, hashPassword, verifyPassword } from "../src/auth/service.mjs";
...
const authError = (fn, code) => assert.throws(fn, (error) => error instanceof AuthError && error.code === code);
try {
  const db = resetAndSeed(filename);
  const auth = createAuthService(db, { clock: () => new Date("2026-01-15T12:00:00.000Z"), sessionLifetimeMs: 60_000 });
  ...
  authError(() => auth.signIn({ email: "owner@northstar.test", password: "wrong password" }), "INVALID_CREDENTIALS");
  authError(() => auth.signIn({ email: "missing@northstar.test", password: "wrong password" }), "INVALID_CREDENTIALS");
  authError(() => auth.requireRole(auth.authenticate(viewer.token), ["owner", "member"]), "FORBIDDEN");
  authError(() => auth.requireOrganization(auth.authenticate(owner.token), "org_outside"), "NOT_FOUND");
