...
async function scan(directory) {
  for (const entry of await readdir(directory, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) await scan(path);
      ...
      if (forbidden.test(source)) throw new Error(`Focused or skipped test is forbidden: ${path}`);
...
console.log(`test suite policy: PASS (${testFiles} files, no focused or skipped tests)`);
